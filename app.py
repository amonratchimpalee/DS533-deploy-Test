...
afc500fb-b94a-4909-a95f-52ae964c2897.png
ทำไมพออัพโหลดรูปแล้ว ชื่อไฟล์รูปไปอยู่ในกล่องอัพโหลกเป็นสีขาว ๆเอาชื่อภาพมาไว้ใต้รูปให้หน่อย

ตอนนี้ Streamlit default file uploader ยังแสดง file chip อยู่ เลยทำให้ชื่อไฟล์ไปโผล่ทับ overlay กลางกล่องอัปโหลด

ให้ซ่อนชื่อไฟล์ใน uploader แล้วเอาไปแสดงใต้รูปแทนแบบนี้จะสวยกว่าและไม่ชน UI

แก้ 2 จุดครับ

1. ซ่อนชื่อไฟล์ในกล่อง upload
เพิ่ม CSS นี้ใน <style> ของคุณ
...
แนะนำใส่ใต้ส่วนนี้เลย:

[data-testid="stFileChipName"],[data-testid="stFileChipName"] *{
    color:rgba(255,255,255,.9)!important;
}
2. แสดงชื่อไฟล์ใต้รูป
เพิ่มใต้:
...
ไม่มีชื่อไฟล์ลอยในกล่อง upload อีก

ชื่อไฟล์ไปอยู่ใต้รูปแทน

UI ดู clean และ luxury มากขึ้น

mobile layout จะไม่พัง

ถ้าอยากให้สวยขึ้นอีก แนะนำแบบนี้:
...
จะได้ card filename แบบ minimal เข้ากับ theme ของ app มากกว่า


แก้ให้หน่อย จากโค้ดเดิมที่ส่งให้

แก้จากโค้ดเดิมของคุณได้แบบนี้เลยครับ

1. เพิ่ม CSS ซ่อนชื่อไฟล์ใน uploader
หา block นี้ใน <style>

[data-testid="stFileChipName"],[data-testid="stFileChipName"] *{
    color:rgba(255,255,255,.9)!important;
    -webkit-text-fill-color:rgba(255,255,255,.9)!important}
แล้วเพิ่มต่อท้ายทันที:

/* ซ่อนชื่อไฟล์ default ของ Streamlit */
[data-testid="stFileUploaderFile"]{
    display:none!important;
}
...
ไม่มีชื่อไฟล์สีขาวลอยในกล่อง upload

ชื่อไฟล์ไปอยู่ใต้รูปแทน

UI ดูสะอาดขึ้น

mobile responsive ดีกว่าเดิม

ไม่ชนกับ overlay 📸 ของคุณอีก
